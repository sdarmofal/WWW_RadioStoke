...
if ( @$_GET['file'] == 'edit' || @$_POST['file'] == 'edit' )
{
	define('MODULE_ID', 62);
}
else if ( @$_GET['file'] == 'delete' || @$_POST['file'] == 'delete' )
{
	define('MODULE_ID', 63);
}
else
...
if( !empty($setmodules) )
{
	...
	$module['Download2']['Afile'] = "$file?file=add";
	$module['Download2']['Efile'] = "$file?file=edit";
	$module['Download2']['Dfile'] = "$file?file=delete";
	return;
}
...
//			}
			...
			if ($upload == 'do')
			{
				if ( ! pafiledb_is_valid_ext(pafiledb_get_extension($userfile_name)) )
				{
					$message = sprintf($lang['extension_forbidden'], pafiledb_get_extension($userfile_name));
					message_die(GENERAL_MESSAGE, $message);
				}

				$template->assign_block_vars("upload_do", array());

				if (file_exists(@phpbb_realpath($phpbb_root_path . "pafiledb/uploads/$userfile_name")))
				{
...
/*				else if ($userfile_size > $MAX_FILE_SIZE)
				{
					$message = $lang['file_not_type'] . '<br /><br />' . sprintf($lang['Click_return'], '<a href="' . append_sid("admin_file.$phpEx?file=upload") . '">', '</a>');
...
*/
				else 
				{
					if (is_uploaded_file($userfile))
					{
						...
						$url = $server_protocol . $server_name . $server_port . $script_name . '/pafiledb/uploads/' . $userfile_name;
					}
					else
					{
						$message = $lang['file_not_type'] . '<br /><br />' . sprintf($lang['Click_return'], '<a href="' . append_sid("admin_file.$phpEx?file=upload") . '">', '</a>');
		...
		case 'add':
		{
			...
			if ($add == 'do')
			{
				...
				$sql = "INSERT INTO " . PA_FILES_TABLE . " (file_name, file_desc, file_creator, file_version, file_longdesc, file_ssurl, file_dlurl, file_time, file_catid, file_posticon, file_license, file_dls, file_last, file_pin, file_docsurl, file_rating, file_totalvotes) VALUES('" . $form['name'] . "', '" . $form['shortdesc'] . "', '" . $form['creator'] . "', '" . $form['version'] . "', '" . $form['longdesc'] . "', '" . $HTTP_POST_VARS['ssurl'] . "', '" . $HTTP_POST_VARS['dlurl'] . "', '" . $time . "', '" . $form['category'] . "', '" . $form['posticon'] . "', '" . $form['license'] . "', '0', '0', '" . $form['pin'] . "', '" . $form['docs'] . "', '0', '1')";
			...
			if (empty($add)) 
			{
				...
				$path = str_replace($phpbb_root_path . "pafiledb/pafiledb.$phpEx", "", $PHP_SELF);
				...
				$template->assign_vars(array(
					...
					'L_NO' => $lang['No'],
					'L_YES' => $lang['Yes'],
					'U_UPLOAD_SS' => "javascript:NewWindow('" . append_sid("admin_file.$phpEx?file=upload&amp;field=ssurl") . "','fileupload','600','450','custom','front');",
					'U_UPLOAD_FILE' => "javascript:NewWindow('" . append_sid("admin_file.$phpEx?file=upload&amp;field=dlurl") . "','fileupload','600','450','custom','front');")
				);
		...
		case 'edit':
		{
			...
			if (empty($edit))
			{
				...
				{
					$row .= '<tr><td width="3%" class="row1" align="center" valign="middle">&nbsp;</td><td width="97%" class="row1"><span class="cattitle"><a href="' . append_sid("admin_file.$phpEx?file=edit&amp;id=" . $cat['cat_id']) . '">' . $cat['cat_name'] . '</a></span><br><span class="gen">' . $cat['cat_desc'] . '</span></td></tr>';
		...
		case 'delete':
		{
			...
			if (empty($delete)) 
			{
				...
				while ($cat = $db->sql_fetchrow($result)) 
				{
					$row .= '<tr><td width="3%" class="row1" align="center" valign="middle">&nbsp;</td><td width="97%" class="row1"><span class="cattitle"><a href="' . append_sid("admin_file.$phpEx?file=delete&amp;id=" . $cat['cat_id']) . '">' . $cat['cat_name'] . '</a></span><br><span class="gen">' . $cat['cat_desc'] . '</span></td></tr>';
