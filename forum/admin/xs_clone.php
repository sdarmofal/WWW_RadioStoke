...
if(!empty($HTTP_GET_VARS['clone']))
{
	...
	$template->assign_vars(array(
			...
			'CLONE_TEMPLATE'	=> xhtmlspecialchars($style),
			'STYLE_ID'			=> $theme_rowset[0]['themes_id'],
			'STYLE_NAME'		=> xhtmlspecialchars($theme_rowset[0]['style_name']),
			'TOTAL'				=> count($theme_rowset),
			'L_CLONE_STYLE3'	=> str_replace('{STYLE}', xhtmlspecialchars($style), $lang['xs_clone_style3'])
			));
	// clone styles
	for($i=0; $i<count($theme_rowset); $i++)
	{
		$template->assign_block_vars('styles', array(
			'ID'		=> $theme_rowset[$i]['themes_id'],
			'TPL'		=> xhtmlspecialchars($theme_rowset[$i]['template_name']),
			'STYLE'		=> xhtmlspecialchars($theme_rowset[$i]['style_name']),
			'L_CLONE'	=> str_replace('{STYLE}', xhtmlspecialchars($theme_rowset[$i]['style_name']), $lang['xs_clone_style2'])
			));
	}
	...
	else
	{
		...
		for($i=0; $i<count($theme_rowset); $i++)
		{
			$template->assign_block_vars('switch_select_style.style', array(
				'NUM'		=> $i,
				'ID'		=> $theme_rowset[$i]['themes_id'],
				'NAME'		=> xhtmlspecialchars($theme_rowset[$i]['style_name'])
				));
		}
	}
...
for($i=0; $i<count($style_rowset); $i++)
{
	...
	{
		$style_names[] = xhtmlspecialchars($item['style_name']);
	}
	else
	{
		if($prev_id > 0)
		...
		$style_names = array(xhtmlspecialchars($item['style_name']));
	}
}
