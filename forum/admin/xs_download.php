...
if(isset($HTTP_GET_VARS['url']) && !defined('DEMO_MODE'))
{
	...
	$str = '<form action="' . $board_config[$var] . '" method="post" style="display: inline;" target="main"><input type="hidden" name="data" value="' . xhtmlspecialchars(serialize($import_data)) . '" /><input type="submit" value="' . $lang['xs_continue'] . '" class="post" /></form>';
...
for($i=0; $i<$board_config['xs_downloads_count']; $i++)
{
	...
	$template->assign_block_vars('url', array(
		...
		'NUM'			=> $i,
		'NUM1'			=> $i + 1,
		'URL'			=> xhtmlspecialchars($board_config['xs_downloads_'.$i]),
		'TITLE'			=> xhtmlspecialchars($board_config['xs_downloads_title_'.$i]),
