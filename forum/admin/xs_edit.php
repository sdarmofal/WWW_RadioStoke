...
$template->assign_vars(array(
	'FILTER_EXT'	=> xhtmlspecialchars($filter_data['ext']),
	'FILTER_DATA'	=> xhtmlspecialchars($filter_data['data']),
	'FILTER_URL'	=> append_sid('xs_edit.'.$phpEx),
	'FILTER_DIR'	=> xhtmlspecialchars($current_dir),
	'S_FILTER'		=> '<input type="hidden" name="filter" value="' . xhtmlspecialchars($filter) . '" />'
	));
...
if(isset($HTTP_GET_VARS['edit']))
{
	...
	$template->assign_vars(array(
		...
		'CURRENT_DIR'	=> xhtmlspecialchars($current_dir_full),
		'DIR'			=> xhtmlspecialchars($current_dir),
		'FILE'			=> xhtmlspecialchars($file),
		'FULLFILE'		=> xhtmlspecialchars($fullfile),
		'CONTENT'		=> defined('DEMO_MODE') ? 'you cannot edit file in demo mode' : xhtmlspecialchars(implode('', @file($localfile))),
		)
	);
	if($current_dir_full)
	{
		$template->assign_block_vars('nav_left',array('ITEM' => '&raquo; <a href="' . append_sid('xs_edit.'.$phpEx.$filter_str.'&dir='.$current_dir) . '">' . xhtmlspecialchars($current_dir_full) . '</a>'));
	}
...
if($current_dir != 'templates')
{
	$template->assign_block_vars('begin_dirs.dir', array(
		'NAME'			=> '..',
		'FULLNAME'		=> xhtmlspecialchars($back_dir ? $back_dir . '/' : ''),
...
for($i=0; $i<count($list_dirs); $i++)
{
	...
	$template->assign_block_vars('begin_dirs.dir', array(
		'NAME'			=> xhtmlspecialchars($dir),
		'FULLNAME'		=> xhtmlspecialchars($current_dir_root . $dir),
		'URL'			=> append_sid('xs_edit.'.$phpEx.$filter_str.'&dir='.urlencode($str)),
		)
	);
}
...
for($i=0; $i<count($list_files_editable); $i++)
{
	...
	$template->assign_block_vars('begin_files.file', array(
		'ROW_CLASS'	=> $row_class,
		'NAME'		=> xhtmlspecialchars($file),
		'FULLNAME'	=> xhtmlspecialchars($fullfile),
