...
if(!empty($HTTP_GET_VARS['importstyle']))
{
	...
	$template->assign_vars(array(
		...
		'S_RETURN'				=> $return_url ? '<input type="hidden" name="return" value="' . xhtmlspecialchars($return_url) . '" />' : '',
		'IMPORT_FILENAME'		=> xhtmlspecialchars($file),
		'STYLE_TEMPLATE'		=> xhtmlspecialchars($header['template']),
		'STYLE_FILENAME'		=> xhtmlspecialchars($file),
		'STYLE_COMMENT'			=> xhtmlspecialchars($header['comment']),
		'DATE'					=> create_date($board_config['default_dateformat'], $header['date'], $board_config['board_timezone'], true),
		'STYLE_SIZE'			=> $header['filesize'],
		'STYLE_NAME'			=> xhtmlspecialchars($header['styles'][0]),
		'TOTAL'					=> count($header['styles']),
		'L_XS_IMPORT_TPL'		=> str_replace('{TPL}', xhtmlspecialchars($header['template']), $lang['xs_import_tpl'])
		));
	if(count($header['styles']) > 1)
	{
		...
		for($i=0; $i<count($header['styles']); $i++)
		{
			$template->assign_block_vars('switch_select_style.style', array(
				'NUM'		=> $i,
				'NAME'		=> xhtmlspecialchars($header['styles'][$i]),
				));
		}
	}
	else
...
if(count($files))
{
	...
	for($i=0; $i<count($files); $i++)
	{
		...
		$template->assign_block_vars('styles', array(
			'ROW_CLASS'		=> $row_class,
			'FILE'			=> xhtmlspecialchars($item['file']),
			'FILE2'			=> xhtmlspecialchars($item['file2']),
			'FILENAME'		=> xhtmlspecialchars($item['filename']),
			'TEMPLATE'		=> xhtmlspecialchars($item['template']),
			'DATE'			=> create_date($board_config['default_dateformat'], $item['date'], $board_config['board_timezone'], true),
			'COMMENT'		=> xhtmlspecialchars($item['comment']),
		...
		{
			$template->assign_block_vars('styles.error', array('ERROR' => xhtmlspecialchars($item['error'])));
		}
	}
}
else
