...
//
if(!empty($HTTP_GET_VARS['moveusers']) && !defined('DEMO_MODE') && !$HTTP_POST_VARS['cancel'])
{
	$id = intval($HTTP_GET_VARS['moveusers']);

	if ( !$HTTP_POST_VARS['confirm'] )
	{
		xs_message($lang['Confirm'], $lang['xs_confirm_users'] . '<form action="' . append_sid('xs_styles.' . $phpEx . '?moveusers=' . $id) . '" method="post"><table border="0"><tr><td class="row1" align="center"><input type="hidden" name="confirm" value="1"><input type="submit" name="confirm" class="mainoption" value="' . $lang['Yes'] . '" />&nbsp;&nbsp;<input type="submit" name="cancel" value="' . $lang['No'] . '" class="liteoption" /></td></tr></table></form>');
	}
...
//
if(!empty($HTTP_GET_VARS['moveaway']) && !defined('DEMO_MODE') && !$HTTP_POST_VARS['cancel'])
{
	$id = intval($HTTP_GET_VARS['moveaway']);
	$id2 = intval($HTTP_GET_VARS['movestyle']);
	if ( !$HTTP_POST_VARS['confirm'] )
	{
		xs_message($lang['Confirm'], $lang['xs_confirm_users'] . '<form action="' . append_sid('xs_styles.' . $phpEx . '?moveaway=' . $id . '&amp;movestyle=' . $id2) . '" method="post"><table border="0"><tr><td class="row1" align="center"><input type="hidden" name="confirm" value="1"><input type="submit" name="confirm" class="mainoption" value="' . $lang['Yes'] . '" />&nbsp;&nbsp;<input type="submit" name="cancel" value="' . $lang['No'] . '" class="liteoption" /></td></tr></table></form>');
	}
	if($id2)
	{
