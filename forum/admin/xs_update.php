...
if(!isset($HTTP_GET_VARS['doupdate']))
{
	...
	foreach($updates as $var => $item)
	{
		...
		$template->assign_block_vars('row',
				...
				'NUM'		=> $counter,
				'VAR'		=> 'item_'.$counter.'_',
				'ITEM'		=> xhtmlspecialchars($var),
				'NAME'		=> xhtmlspecialchars($item['update_name']),
				'TYPE'		=> $lang['xs_update_types'][$type],
				'URL'		=> xhtmlspecialchars($item['update_url']),
				'VERSION'	=> xhtmlspecialchars($item['update_version'])
				)
			);
...
foreach($updates as $var => $item)
{
	if(isset($item['data']) && is_array($item['data']))
	{
		$count_total++;
		$type = isset($lang['xs_update_types'][$item['update_type']]) ? $item['update_type'] : 0;
		$ver1 = xhtmlspecialchars($item['update_version']);
		$row_class = $xs_row_class[$count_total % 2];
		$template->assign_block_vars('row',
			array(
				'ROW_CLASS'		=> $row_class,
				'ITEM'			=> xhtmlspecialchars($item['update_name']),
				'TYPE'			=> $lang['xs_update_types'][$type],
				'VERSION'		=> $ver1,
			)
		);
		...
		{
			$ver2 = xhtmlspecialchars($item['data']['version']);
			...
			if($ver2 !== $ver1 && (!empty($item['data']['update']) || !empty($item['data']['autoupdate'])))
			{
				...
				$template->assign_block_vars('row.update',
					array(
						'NUM'			=> $count_total,
						'VERSION'		=> $ver2,
						'UPDATE'		=> isset($item['data']['update']) ? xhtmlspecialchars($item['data']['update']) : '',
						'U_IMPORT'		=> $u_import,
						'INFO'			=> xhtmlspecialchars($info),
					)
				);
			...
			else
			{
				$template->assign_block_vars('row.noupdate', 
					array(
						...
						'INFO'			=> empty($info) ? '' : xhtmlspecialchars($info),
					)
				);
		...
		else
		{
			...
			$template->assign_block_vars('row.error', array('ERROR' => xhtmlspecialchars($item['data']['error'])));
			$count_error++;
		}
	}
}
