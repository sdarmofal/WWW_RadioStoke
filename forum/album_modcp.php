...
{
	$pic_post_id = $HTTP_POST_VARS['pic_id'];
}
...
// Get the cat info
// ------------------------------------
if( ($cat_id == 0) and (($HTTP_GET_VARS['mode'] == 'lock') or ($HTTP_GET_VARS['mode'] == 'unlock')) )
{
...
if ($mode == '')
{
	...
	if ($total_pics > 0)
	{
		...
		generate_pagination(append_sid("album_modcp.$phpEx?cat_id=$cat_id&amp;sort_method=$sort_method&amp;sort_order=$sort_order"), $total_pics, $pics_per_page, $start);
	}
	else
	{
		// No Pics
...
else
{
	...
	if ($mode == 'move')
	{
		...
		if( !isset($HTTP_POST_VARS['target']) )
		{
			...
			else
			{
				...
				if( isset($pic_post_id) )
				{
					$pic_id_array = $pic_post_id;
		...
		else
		{
			...
			if( isset($pic_post_id) )
			{
				...
				if( is_array($pic_id) )
				{
					...
					{
						$pic_id_sql .= (($pic_id_sql) ? ', ' : '') . intval($pic_id[$k]);
					}
					if ( !$pic_id_sql )
					{
						message_die(GENERAL_ERROR, 'No pics selected');
					}
				}
				else
				{
	...
	else if ($mode == 'delete')
	{
		...
		if( !isset($HTTP_POST_VARS['confirm']) )
		{
			...
			for ($i = 0; $i < count($pic_id_array); $i++)
			{
				$hidden_field .= '<input name="pic_id[]" type="hidden" value="'. intval($pic_id_array[$i]) .'" />' . "\n";
			}
