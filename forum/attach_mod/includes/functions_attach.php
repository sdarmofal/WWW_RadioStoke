...
function physical_filename_already_stored($filename)
{
	...
	{
		message_die(GENERAL_ERROR, 'Could not get attachment information for filename: ' . xhtmlspecialchars($filename), '', __LINE__, __FILE__, $sql);
	}
...
function attachment_exists_db($post_id, $page = 0)
{
	...
	$post_id = (is_array($post_id)) ? implode(',',array_filter(array_map('intval',$post_id))) : intval($post_id);
	$sql_id  = ($page == PAGE_PRIVMSGS) ? 'privmsgs_id' : 'post_id';

	$sql     = 'SELECT attach_id, '.$sql_id.' FROM ' . ATTACHMENTS_TABLE . " WHERE $sql_id IN($post_id)";
	$result  = $db->sql_query($sql) or message_die(GENERAL_ERROR, 'Could not get attachment informations for specific posts', '', __LINE__, __FILE__, $sql);
	
	$match   = array();
	while($row = $db->sql_fetchrow($result))
	{
        $match[$row[$sql_id]] = true;
    }
...
function _set_var(&$result, $var, $type, $multibyte = false)
{
	...
	if ($type == 'string')
	{
		$result = trim(xhtmlspecialchars(str_replace(array("\r\n", "\r", '\xFF'), array("\n", "\n", ' '), $result)));
