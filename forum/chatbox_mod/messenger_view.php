...
function add_msg($nick, $msg) // This function processes the submission from user - Smartor
{
	...
	$nick = stripslashes(xhtmlspecialchars(trim($nick)));
	$msg = xhtmlspecialchars(trim($msg));
...
function chatters_sub() // This function checks status of user
{
	...
	for($s = 1; $s <= $isimler[0]; $s++)
	{
		...
		if($currentact != $activity[$s])
		{
			...
			$sql = "UPDATE " . $table_chatbox_session_name . " SET laststatus = '" . $db->sql_escape($currentact) . "' WHERE username = \"" . $db->sql_escape($isimler[$s]) . "\"";
