...
$group_id = get_vars(POST_GROUPS_URL, 0, 'POST,GET');
...
else if ( $group_id )
{
	...
	//
	$sql = "SELECT username, user_id, user_viewemail, user_posts, user_regdate, user_from, user_website, user_email, user_icq, user_yim, user_msnm, user_jr, user_level  
		FROM " . USERS_TABLE . " 
    ...
    $modgroup_pending_list=$group_members=array();
    $modgroup_pending_count=$members_count=$is_group_member=$is_group_pending_member=0;

    $is_autogroup_enable = ($group_info['group_count'] <= $userdata['user_posts']) ? true : false;

    $sql = "SELECT u.username, u.user_id, u.user_level, u.user_jr, u.user_viewemail, u.user_posts, u.user_regdate, u.user_from, u.user_website, u.user_email, u.user_icq, u.user_yim, u.user_msnm,ug.user_pending
    ...
    $result = $db->sql_query($sql) or message_die(GENERAL_ERROR, 'Error getting user list for group', '', __LINE__, __FILE__, $sql);
    while($row = $db->sql_fetchrow($result))
    {
        if($row['user_pending'])
        {
            $modgroup_pending_list[] = $row;
            $modgroup_pending_count += 1;

            if ( $row['user_id'] == $userdata['user_id'] && $userdata['session_logged_in'] )
            {
    ...
    $mod_username = color_username($group_moderator['user_level'], $group_moderator['user_jr'], $user_id, $username);
	$template->assign_vars(array(
        ...
        'MOD_USERNAME'       => $mod_username[0],
        'MOD_USERNAME_STYLE' => $mod_username[1],
		'MOD_FROM' => $from,
		'MOD_JOINED' => $joined,
		'MOD_POSTS' => $posts,
	...
	for($i = $start; $i < min($user_topics_per_page + $start, $members_count); $i++)
	{
		...
		if ( $group_info['group_type'] != GROUP_HIDDEN || $is_group_member || $is_moderator )
		{
			...
			$colored_member = color_username($member_row['user_level'], $member_row['user_jr'], $user_id, $username);
			$template->assign_block_vars('member_row', array(
				'ROW_COLOR'      => '#' . $row_color,
				'ROW_CLASS'      => $row_class,
				'USERNAME'       => $colored_member[0],
				'USERNAME_STYLE' => $colored_member[1],
				'FROM' => $from,
				'JOINED' => $joined,
				'POSTS' => $posts,
				'USER_ID' => $user_id, 
	...
	if ( $is_moderator )
	{
		...
		if ( $modgroup_pending_count )
		{
			for($i = 0; $i < $modgroup_pending_count; $i++)
			{
				...
				$colored_pending_members_row = color_username($modgroup_row['user_level'], $modgroup_row['user_jr'], $user_id, $username);
				$template->assign_block_vars('pending_members_row', array(
					'ROW_CLASS'      => $row_class,
					'ROW_COLOR'      => '#' . $row_color, 
					'USERNAME' 		 => $colored_pending_members_row[0],
					'USERNAME_STYLE' => $colored_pending_members_row[1],
					'FROM' => $from,
					'JOINED' => $joined,
					'POSTS' => $posts,
					'USER_ID' => $user_id, 
