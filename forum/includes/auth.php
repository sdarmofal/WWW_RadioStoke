...
/*
	...
	* If you set forum_id to AUTH_LIST_ALL and type to AUTH_ALL a multidimensional
	...
	lookup
*/
function auth($type, $forum_id, $userdata, $f_access = array())
{
	global $db, $lang;
	...
	if ( empty($f_access) )
	{
        $f_access = sql_cache('check', 'f_access');
        if( empty($f_access) )
        {
            $f_access = array();
            $sql = "SELECT forum_id, auth_view, auth_read, auth_post, auth_reply, auth_edit, auth_delete,
                auth_sticky, auth_announce, auth_globalannounce, auth_vote, auth_pollcreate, auth_attachments,
                auth_download FROM " . FORUMS_TABLE;
            $result = $db->sql_query($sql) or message_die(GENERAL_ERROR, 'Failed obtaining forum access control lists', '', __LINE__, __FILE__, $sql);
            while( $row = $db->sql_fetchrow($result) )
            {
                $f_access[$row['forum_id']] = $row;
            }
            $db->sql_freeresult($result);

            if ( empty($f_access) && !defined('IN_ADMIN') && !defined('IN_LOGIN') )
            {
                message_die(GENERAL_MESSAGE, $lang['No_forums']);
            }
            sql_cache('write', 'f_access', $f_access);
        }
	}
    ...
    if ( $forum_id != AUTH_LIST_ALL )
    {
        ...
        {
            $key = $auth_fields[$i];
            $value = isset($f_access[$key]) ? $f_access[$key] : $f_access[$forum_id][$key];
    ...
    elseif( is_array($f_access) )
    {
        foreach( $f_access as $k => $v )
        {
            $f_forum_id = $v['forum_id'];
            $_u_access  = isset($u_access[$f_forum_id]) ? $u_access[$f_forum_id] : array();
            ...
            for($i=0; $i<$a_f; $i++)
            {
                ...
                switch( $value )
                {
                    ...
                    case AUTH_ACL:
                        $auth_user[$f_forum_id][$key] = ( $userdata['session_logged_in'] ) ? auth_check_user(AUTH_ACL, $key, $_u_access, $is_admin) : 0;
                        $auth_user[$f_forum_id][$key . '_type'] = $lang['Auth_Users_granted_access'];
                        break;

                    case AUTH_MOD:
                        $auth_user[$f_forum_id][$key] = ( $userdata['session_logged_in'] ) ? auth_check_user(AUTH_MOD, 'auth_mod', $_u_access, $is_admin) : 0;
                ...
                // Is user a moderator?
               $auth_user[$f_forum_id]['auth_mod'] = ( $userdata['session_logged_in'] ) ? auth_check_user(AUTH_MOD, 'auth_mod', $_u_access, $is_admin) : 0;
            }
        }
    }
