 ...
 *   date modification    : ver. 1.12.0 2005/10/09 16:33
 *
...
//	$username = preg_replace('#\s+#', ' ', trim($username)); 
	...
	// phpBB don't right operating chars as "&<>\
	if (strstr($username, '"') || strstr($username, '&') || strstr($username, chr(160)) || strstr($username, chr(173)) || strstr($username, '  ') || strstr(stripslashes($username), '\\') || strstr(stripslashes($username), '%') || strstr(stripslashes($username), '*')  )
	{
...
//
function validate_optional_fields(&$icq, &$aim, &$msnm, &$yim, &$website, &$location, &$occupation, &$interests, &$custom_color, &$sig)
{
