...
{ 
	$locbar = '<span class="nav"><a href="' . append_sid("index.$phpEx") . '" class="nav">' . $board_config['sitename'] . '</a> &raquo; <a href="' . append_sid("dload.php") . '" class="nav">Download</a> &raquo; ' . $category['cat_name'] . '</span>';
}
...
if ($category['cat_parent'] > 0)
{
	...
	$locbar = '<span class="nav"><a href="' . append_sid("index.$phpEx") . '" class="nav">' . $board_config['sitename'] . '</a> &raquo; <a href="' . append_sid("dload.php") . '" class="nav">Download</a> &raquo; <a href="' . append_sid("dload.php?action=category&amp;id=" . $parent['cat_id']) . '" class="nav">' . $parent['cat_name'] . '</a> &raquo; ' . $category['cat_name'] . '</span>';
}
...
if ($row_number > 0) 
{
	...
	while ($sub = $db->sql_fetchrow($result)) 
	{
		...
		if (CR_TIME - ($config['settings_newdays'] * 24 * 60 * 60) < $file_new['file_time'])
		{
...
while ($pin = $db->sql_fetchrow($result)) 
{
	...
	if (CR_TIME - ($config['settings_newdays'] * 24 * 60 * 60) < $pin['file_time'])
	{
		$newind = '<img src="pafiledb/images/new.gif" border="0" alt="New file" />';
	}

	$filelist .= '<tr> 
			<td class="row1" align="center" valign="middle"><a href="' . append_sid("dload.php?action=file&amp;id=" . $pin['file_id']) . '" class="topictitle"><img src="pafiledb/images/pin.gif" border="0" alt="" /></a></td>
...
while ($file = $db->sql_fetchrow($result)) 
{
	...
	{
		$posticon = '<img src="pafiledb/icons/' . $file['file_posticon']. '" border="0" alt="" />';
	}

	$newind = '';

	if (CR_TIME - ($config['settings_newdays'] * 24 * 60 * 60) < $file['file_time'])
	{
		$newind = '<img src="pafiledb/images/new.gif" border="0" alt="New file" />';
	}
