...
$locbar = '<span class="nav"><a href="' . append_sid("index.$phpEx") . '" class="nav">' . $board_config['sitename'] . '</a> &raquo; <a href="' . append_sid("dload.php") . '" class="nav">Download</a></span>';
...
while ($category = $db->sql_fetchrow($result))
{
	...
	if ($category['cat_parent'] == 0) 
	{
		...
		while($category_file = $db->sql_fetchrow($subgroups)) 
		{
			...
			if (CR_TIME - ($config['settings_newdays'] * 24 * 60 * 60) < $file_in_sub['file_time'])
			{
				$newind = '<img src="' . $images['folder_new'] . '" border="0" alt="New file" />';
