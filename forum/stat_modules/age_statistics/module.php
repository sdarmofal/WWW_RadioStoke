...
if (!$statistics->result_cache_used)
{
	...
	if ( !$result )
	{
		message_die(GENERAL_ERROR, "Could not find posts.", "",__LINE__, get_module_fd_name(__FILE__), $sql);
	}
	...
	if ( !$result )
	{
	//	  message_die(GENERAL_ERROR, "Could not find users.", "",__LINE__, get_module_fd_name(__FILE__), $sql);
	}
