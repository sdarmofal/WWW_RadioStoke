...
if (!$statistics->result_cache_used)
{
	...
	{
		message_die(GENERAL_ERROR, 'Couldn\'t retrieve posts data', '', __LINE__, get_module_fd_name(__FILE__), $sql);
	}
	...
	for ($i = 0; $i < $posts_count; $i=$i+$k)
	{
		...
		$result_cache->assign_template_block_vars('newposts');
	}
}
else
{
	for ($i = 0; $i < $result_cache->block_num_vars('newposts'); $i++)
	{
		// Method 1: We are assigning the block variables from the result cache to the template. ;)
		$template->assign_block_vars('newposts', $result_cache->get_block_array('newposts', $i));
